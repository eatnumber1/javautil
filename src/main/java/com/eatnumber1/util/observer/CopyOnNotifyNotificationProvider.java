 ...
 * @since Jul 27, 2009
 */
public class CopyOnNotifyNotificationProvider<T> implements ObservableNotificationProvider<T> {
    @Override
    public void notifyObservers( @NotNull Observable<T> observable, @NotNull SynchronizedCollection<Observer<T>, ? extends Collection<Observer<T>>> observers, @Nullable T arg ) {
        Collection<Observer<T>> copiedObservers;
        Lock readLock = observers.readLock();
        readLock.lock();
        try {
            copiedObservers = new ArrayList<Observer<T>>(observers);
        } finally {
            readLock.unlock();
        }
        for( Observer<T> observer : copiedObservers ) {
